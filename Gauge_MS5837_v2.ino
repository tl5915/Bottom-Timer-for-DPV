...
unsigned long batterySampleCount = 0;
...
void loop() {
  ...
  if (currentTime - prevDisplayTime >= 500) {  // 2Hz refresh rate
    ...
    if (batterySampleCount > 0) {
      float averageReading = batterySum / (float)batterySampleCount;
      batteryVoltage = averageReading * (3.3 / 4095.0) * 2;  // 1:1 voltage divider
    } else {
      batteryVoltage = 0;
    }
